...
- **API ASR option**: OpenAI-compatible audio transcription for machines that should not load local ASR models
...
- **Manual text translation**: Type or paste text in Settings with literal, natural, explain, and polish modes, with history saving
...
The Chrome selection-translation extension lives in `browser-extension/`. See [docs/chrome_extension_install.md](docs/chrome_extension_install.md).

### Windows Packaging

Release packaging notes are in [docs/packaging.md](docs/packaging.md). Quick build:
...
Run the basic non-GUI check without loading ASR models:
...
1. Setup wizard appears — choose download source (ModelScope / HuggingFace) and model cache folder
...
The local selection-translation API token is also configured on the Translation tab. If set, enter the same token in the Chrome extension popup; the extension never stores API keys.

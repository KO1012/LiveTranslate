...
- **流式翻译显示**：翻译结果逐字实时显示
- **手动文本翻译**：在设置页直接输入文本，支持直译、自然表达、解释、润色并保存历史
...
Chrome 右键划词翻译插件在 `browser-extension/`，安装说明见 [docs/chrome_extension_install.md](docs/chrome_extension_install.md)。

### Windows 打包

发布打包说明见 [docs/packaging.md](docs/packaging.md)。快速构建：
...
1. 弹出设置向导——选择下载源（ModelScope 适合国内，HuggingFace 适合海外）和模型缓存目录
...
本地划词翻译接口的 Token 也在翻译标签页配置。若设置了 Token，Chrome 插件 popup 中需要填写同一个值；插件不会保存 API Key。

...
class AudioCapture:
    ...
    def _open_stream(self):
        ...
        max_channels = int(loopback_dev.get("maxInputChannels") or 1)
        channel_candidates = []
        for channels in (max_channels, 2, 1):
            if channels > 0 and channels <= max_channels and channels not in channel_candidates:
        ...
        for channels in channel_candidates:
            try:
                ...
                log.info(
                    f"Native: {self._native_rate}Hz, {self._native_channels}ch "
                ...
                last_error = e
                log.warning(
                    f"Failed to open loopback with {channels}ch on "
                    f"{loopback_dev['name']}: {e}"
                )

        raise last_error or RuntimeError("Failed to open WASAPI loopback stream")

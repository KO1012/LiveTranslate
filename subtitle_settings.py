...
class SubtitleSettingsWidget(QWidget):
    ...
    def sync_external_state(self, partial: dict):
        """Merge externally-managed keys (e.g. enabled / window_x / window_y set
        by the tray toggle or window drag) into the internal settings without
        rebuilding the UI or re-emitting. Prevents a later style change from
        emitting a stale ``enabled``/position and silently reverting them.
        """
        if partial:
